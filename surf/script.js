    ...
    for (var i = 0; i < elems.length; i++) {
        ...
        if ( elem_bottom >= win_top && elem_top <= win_bottom) {
            ...
            span.style.cssText = [
                ...
                'color: ' + (hint_open_in_new_tab ? '#6f5d8a' : '#a8a8a8') + ';',
                'font-weight: bold;',
                'z-index: 100000;'
                    ].join('');
...
function initKeyBind(e){
    ...
    if( t.nodeType == 1){
        ...
        addKeyBind( 'C-t', 'hintMode(true)', e );
        addKeyBind( 'C-l', 'removeHints()', e );
    }
}
...
function bottom() {
	...
	window.scroll(document.width, max([body.scrollHeight, body.offsetHeight, html.clientHeight, html.scrollHeight, html.offsetHeight ]));
}
